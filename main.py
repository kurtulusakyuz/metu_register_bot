...
def main():
    ...
    course_code, section, user_code, password, course_category = takeInputs()
    if opening_time:
        try:
            ...
            metu.registerWaiting(course_code, section, course_category, target_time_utc, user_code=user_code, password=password)
        except ValueError:
        ...
        if metu.logged_in:
            ...
            metu.registerContinously(course_code, section, course_category, total_attempts=config.TOTAL_ATTEMPTS, avg_jitter=jitter)
        else:
...
def takeInputs():
    ...
    8: NONTECHNICAL ELECTIVE
    9: NOT INCLUDED
              ''')
        course_category = int(input('Please select course category.'))
    except ValueError:
        logger.error('Course code and section must be integers.')
        sys.exit(1)
    user_code = input('Please enter your usercode: ')
    password = getpass.getpass('Please enter the password: ')
    return course_code, section, user_code, password , course_category
